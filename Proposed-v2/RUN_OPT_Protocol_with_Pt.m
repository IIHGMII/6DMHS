...
% Run the 3-stage protocol for one transmit power point.
% Pt: transmit power (dBm)
% K_rice: Rician K-factor (linear)
...
% Default to a small array (matches fig5.m). Change to 32/32 for paper-scale runs.
Mx = 6; My = 6; M = Mx * My;
K = 4; B = 4;
...
% Ring boundary sensing points (required by current Sensing_Algorithm)
...
% Stage I: init + channel + sensing
[Coor_Ele, normal_vector, Rot_Matrix, ~, Coor_Ele_r, Coor_Ele_c, ...
 Coor_Ele_x_d, Coor_Ele_x_u, Coor_Ele_y_l, Coor_Ele_y_r] = ...
    Orientation_Initial(B, Coor_Ele_init, Coor_Ele_init_r, Coor_Ele_init_c, 0, ...
                        Coor_Ele_init_x_d, Coor_Ele_init_x_u, Coor_Ele_init_y_l, Coor_Ele_init_y_r);

[~, e0, e2, ~, ~, ~, ~, Iota, eta, Omega, h_r, h_c, h_sen, ...
 h_x_d, h_x_u, h_y_l, h_y_r, Omega0] = ...
    Channel_Generation_init(K, B, M, Mx, My, M_x_r, M_y_c, normal_vector, K_rice, ...
                            Coor_Ele, Coor_Ele_r, Coor_Ele_c, lambda, Rot_Matrix, ...
                            Coor_Ele_x_d, Coor_Ele_x_u, Coor_Ele_y_l, Coor_Ele_y_r);

[est, ~, ~] = Sensing_Algorithm(K, B, Iota, Mx, My, M_x_r, M_y_c, ...
                                h_r, h_c, Rot_Matrix, e0, K_rice, lambda, e2, ...
                                sigma0, Omega, eta, N, d, kappa0, ...
                                h_sen, h_x_d, h_x_u, h_y_l, h_y_r, ...
                                Coor_Ele_init_x_d, Coor_Ele_init_x_u, Coor_Ele_init_y_l, Coor_Ele_init_y_r, ...
                                Omega0);

% Stage II: rotation alignment (Rodrigues)
normal_vector_rot = cell(K, 1);
for k = 1:K
...
% Stage II: translation + beamforming (demo implementation)
[s, ~, mm, ~] = optimize_system(B, K, M, Mx, My, est, ...
                                normal_vector_rot, Rot_Matrix, Coor_Ele_init, ...
                                lambda, dx, dy, delta_x, delta_y, dmin, V_F0, V_F, Ptx);

% Stage III: equivalent channel + SWIPT optimization
H_eff = generate_effective_channel(s, K, B, M, Iota, Coor_Ele, est, ...
                                   normal_vector_rot, Rot_Matrix, Coor_Ele_init, ...
                                   e0, e2, eta, K_rice, Omega0, lambda, ...
                                   delta_x, delta_y, dx, dy, mm, V_F);
