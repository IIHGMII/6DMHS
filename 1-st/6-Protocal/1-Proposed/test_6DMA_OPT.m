...
rho  = 0.8 * rand(K,1); %功率分割因子,rho用于传能，(1-rho)用于通信
P_out_ite = [];
...
for b = 1:B
    for k = 1:K
        H_eff( (b-1)*Q+1 : b*Q , k ) = ( H( (b-1)*M+1 : b*M , k ).' * diag( V_F ) * mm(:,b) ).' ;
    end
end
 Xtr = randn(Q*B,K);
...
variable mm(M,B)
variable R0(1)
expressions mm_temp1(M,K,B) R_temp(K,K) R_thro(K,1) R_temp0(K,1) H_eff(B*Q,K)
...
%     psi2 = psi2_fixed;
    ...
    [P_ave(iiiii)] = fun_run_R0000( R00 , H_eff , rho , psi1 , psi2_fixed , sigma0 , Ptx , B , K , Q , M , H , A_ST , V_F , Xtr , eta0 );
