% 6DMA,使用全息感知方法
% main1.m函数用于求6DMA空间姿态
% 求解完空间姿态后，需要用其他函数来求数字波束赋型
% 4个全息超表面
clc;clear;

K_rice = 10;
% for num = 1:1
%     [P_out(num)] = test_6DMA_Proposed(K_rice)
% end
% 
% max(P_out)
% 
...
% 初始化优化参数
norm_coff = 1;
psi1 = 0.005 * randn(K,K) + 0.005j * randn(K,K);   psi2 = 0.1 * rand(K,1) + 0.1j * rand(K,1) ;
rho  = 0.001 * rand(K,1); %功率分割因子,rho用于传能，(1-rho)用于通信
P_out_ite = [];
for ite_num = 1:20
