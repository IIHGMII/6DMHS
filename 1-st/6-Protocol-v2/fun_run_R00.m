...
for k = 1:K
    ...
    % end
    R_thro(k)      = log( 1 + 2 * sqrt(1-rho(k)) * real( psi2(k)' * Xtr(:,k)' * conj( H_eff0(:,k) ) ) - abs(psi2(k))^2 * ( (1-rho(k)) * sum( R_temp(:,k) ) + sigma0 ) ) / log(2);
...
for k = 1:K
    ...
    psi2(k)   = sqrt(1-rho(k)) * Xtr(:,k)' * conj( H_eff0(:,k) ) / ( (1-rho(k)) * sum( R_temp(:,k) ) + sigma0 ) ;
end
