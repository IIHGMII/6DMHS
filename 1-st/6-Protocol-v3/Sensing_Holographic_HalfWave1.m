...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%使用穷举搜索，找到感知角度。
% angle_set_0 = -1:2/Mx:1;    angle_set_1 = -1:2/My:1;
angle_set_0 = -1:0.01:1;    angle_set_1 = -1:0.01:1;
            ...
            for d_y0 = angle_set_1
                if d_x0^2 + d_y0^2 <=1
                    ...
                    if normal_vector{b}.' * e_erg0 >=0
                        % 第三项为正数
                        b_temp1 = b_steering{b}(e_erg0);
                        b_temp2 = reshape( b_temp1 , Mx , My );
                        h_temp  = b_temp2( Mx0 , My0 );
                        H_rec0{b,k}(num1,num2) = abs( vec(h_temp).' * vec( T{k,b} ) );
                    else
                        H_rec0{b,k}(num1,num2) = 0;
                    end
                    ...
                    if normal_vector{b}.' * e_erg1 >=0
                        ...
                        b_temp2 = reshape( b_temp1 , Mx , My );
                        h_temp  = b_temp2( Mx0 , My0 );
                        H_rec1{b,k}(num1,num2) = abs( vec(h_temp).' * vec( T{k,b} ) );
                    else
                        H_rec1{b,k}(num1,num2) = 0;
                    end
