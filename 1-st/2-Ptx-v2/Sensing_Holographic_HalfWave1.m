...
for b = 1:B, x_ref{k,b} = sqrt(1e-2) * exp( 1j * rand(Mx/2,My/2)*2*pi ); end
end

%全息图，存储在计算机中
for k = 1:K
    for b = 1:B
        n{k,b} = sqrt(1e-11) * ( randn(Mx/2,My/2) + 1j * randn(Mx/2,My/2) );
...
delta_x = ( 2 * [0:((Mx/2)-1)].' - (Mx/2) + 1 )/2; delta_y = ( 2 * [0:( (Mx/2)-1)].' - (Mx/2) + 1 )/2;
F =  exp( -1j * 2*pi * delta_x * delta_y.' / (Mx/2) );
