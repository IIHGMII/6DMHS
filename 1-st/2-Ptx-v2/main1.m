            ...
            for m4 = C_set{4}(C_set{4}~=m1 & C_set{4}~=m2 & C_set{4} ~=m3)
                for m5 = C_set{5}(C_set{5}~=m1 & C_set{5}~=m2 & C_set{5} ~=m3 & C_set{5}~=m4 )
                    for m6 = C_set{6}(C_set{6}~=m1 & C_set{6}~=m2 & C_set{6} ~=m3 & C_set{6}~=m4 & C_set{6}~=m5 )
                        for m7 = C_set{7}(C_set{7}~=m1 & C_set{7}~=m2 & C_set{7} ~=m3 & C_set{7}~=m4 & C_set{7}~=m5 & C_set{7}~=m6 )
                            for m8 = C_set{8}(C_set{8}~=m1 & C_set{8}~=m2 & C_set{8} ~=m3 & C_set{8}~=m4 & C_set{8}~=m5 & C_set{8}~=m6 & C_set{8}~=m7 )

                                if s_dic(m1,1,1,1)  + s_dic(m2,1,2,2) + s_dic(m3,1,3,3) + s_dic(m4,1,4,4) + s_dic(m5,1,5,5) + s_dic(m6,1,6,6) + s_dic(m7,1,7,7) + s_dic(m8,1,8,8) + ...
                                   s_dic(m1,m2,1,2) + s_dic(m1,m3,1,3) + s_dic(m1,m4,1,4) + s_dic(m1,m5,1,5) + s_dic(m1,m6,1,6) + s_dic(m1,m7,1,7) + s_dic(m1,m8,1,8) + ...
                                   s_dic(m2,m1,2,1) + s_dic(m2,m3,2,3) + s_dic(m2,m4,2,4) + s_dic(m2,m5,2,5) + s_dic(m2,m6,2,6) + s_dic(m2,m7,2,7) + s_dic(m2,m8,2,8) + ...
                                   s_dic(m3,m1,3,1) + s_dic(m3,m2,3,2) + s_dic(m3,m4,3,4) + s_dic(m3,m5,3,5) + s_dic(m3,m6,3,6) + s_dic(m3,m7,3,7) + s_dic(m3,m8,3,8) + ...
                                   s_dic(m4,m1,4,1) + s_dic(m4,m2,4,2) + s_dic(m4,m3,4,3) + s_dic(m4,m5,4,5) + s_dic(m4,m6,4,6) + s_dic(m4,m7,4,7) + s_dic(m4,m8,4,8) + ...
                                   s_dic(m5,m1,5,1) + s_dic(m5,m2,5,2) + s_dic(m5,m3,5,3) + s_dic(m5,m4,5,4) + s_dic(m5,m6,5,6) + s_dic(m5,m7,5,7) + s_dic(m5,m8,5,8) + ...
                                   s_dic(m6,m1,6,1) + s_dic(m6,m2,6,2) + s_dic(m6,m3,6,3) + s_dic(m6,m4,6,4) + s_dic(m6,m5,6,5) + s_dic(m6,m7,6,7) + s_dic(m6,m8,6,8) + ...
                                   s_dic(m7,m1,7,1) + s_dic(m7,m2,7,2) + s_dic(m7,m3,7,3) + s_dic(m7,m4,7,4) + s_dic(m7,m5,7,5) + s_dic(m7,m6,7,6) + s_dic(m7,m8,7,8) + ...
                                   s_dic(m8,m1,8,1) + s_dic(m8,m2,8,2) + s_dic(m8,m3,8,3) + s_dic(m8,m4,8,4) + s_dic(m8,m5,8,5) + s_dic(m8,m6,8,6) + s_dic(m8,m7,8,7)   ...
                                   == 64
                                   LoopUntil = 1;
                                   break;
                                end
...
if LoopUntil == 1
    s(m1,1) = 1;s(m2,2) = 1;s(m3,3) = 1;s(m4,4) = 1;s(m5,5) = 1;s(m6,6) = 1;s(m7,7) = 1;s(m8,8) = 1;
else 
...
tic
            mm1 = sparse( diag( m_temp(:,1) ) ); mm2 = sparse( diag( m_temp(:,2) ) ); mm3 = sparse( diag( m_temp(:,3) ) ); mm4 = sparse( diag( m_temp(:,4) ) ); mm5 = sparse( diag( m_temp(:,5) ) ); mm6 = sparse( diag( m_temp(:,6) ) ); mm7 = sparse( diag( m_temp(:,7) ) ); mm8 = sparse( diag( m_temp(:,8) ) );
            gain_beam_temp(:,:,k0) = [  a_st_temp(:,k0,1).' * mm1 *  V_F * Xtr(1,1) , a_st_temp(:,k0,1).' * mm1 *  V_F * Xtr(2,1) , a_st_temp(:,k0,1).' * mm1 *  V_F * Xtr(3,1) , a_st_temp(:,k0,1).' * mm1 *  V_F * Xtr(4,1) , a_st_temp(:,k0,1).' * mm1 *  V_F * Xtr(5,1) , a_st_temp(:,k0,1).' * mm1 *  V_F * Xtr(6,1) , a_st_temp(:,k0,1).' * mm1 *  V_F * Xtr(7,1) , a_st_temp(:,k0,1).' * mm1 *  V_F * Xtr(8,1)
                                        a_st_temp(:,k0,2).' * mm2 *  V_F * Xtr(1,2) , a_st_temp(:,k0,2).' * mm2 *  V_F * Xtr(2,2) , a_st_temp(:,k0,2).' * mm2 *  V_F * Xtr(3,2) , a_st_temp(:,k0,2).' * mm2 *  V_F * Xtr(4,2) , a_st_temp(:,k0,2).' * mm2 *  V_F * Xtr(5,2) , a_st_temp(:,k0,2).' * mm2 *  V_F * Xtr(6,2) , a_st_temp(:,k0,2).' * mm2 *  V_F * Xtr(7,2) , a_st_temp(:,k0,2).' * mm2 *  V_F * Xtr(8,2)
                                        a_st_temp(:,k0,3).' * mm3 *  V_F * Xtr(1,3) , a_st_temp(:,k0,3).' * mm3 *  V_F * Xtr(2,3) , a_st_temp(:,k0,3).' * mm3 *  V_F * Xtr(3,3) , a_st_temp(:,k0,3).' * mm3 *  V_F * Xtr(4,3) , a_st_temp(:,k0,3).' * mm3 *  V_F * Xtr(5,3) , a_st_temp(:,k0,3).' * mm3 *  V_F * Xtr(6,3) , a_st_temp(:,k0,3).' * mm3 *  V_F * Xtr(7,3) , a_st_temp(:,k0,3).' * mm3 *  V_F * Xtr(8,3)
                                        a_st_temp(:,k0,4).' * mm4 *  V_F * Xtr(1,4) , a_st_temp(:,k0,4).' * mm4 *  V_F * Xtr(2,4) , a_st_temp(:,k0,4).' * mm4 *  V_F * Xtr(3,4) , a_st_temp(:,k0,4).' * mm4 *  V_F * Xtr(4,4) , a_st_temp(:,k0,4).' * mm4 *  V_F * Xtr(5,4) , a_st_temp(:,k0,4).' * mm4 *  V_F * Xtr(6,4) , a_st_temp(:,k0,4).' * mm4 *  V_F * Xtr(7,4) , a_st_temp(:,k0,4).' * mm4 *  V_F * Xtr(8,4)
                                        a_st_temp(:,k0,5).' * mm5 *  V_F * Xtr(1,5) , a_st_temp(:,k0,5).' * mm5 *  V_F * Xtr(2,5) , a_st_temp(:,k0,5).' * mm5 *  V_F * Xtr(3,5) , a_st_temp(:,k0,5).' * mm5 *  V_F * Xtr(4,5) , a_st_temp(:,k0,5).' * mm5 *  V_F * Xtr(5,5) , a_st_temp(:,k0,5).' * mm5 *  V_F * Xtr(6,5) , a_st_temp(:,k0,5).' * mm5 *  V_F * Xtr(7,5) , a_st_temp(:,k0,5).' * mm5 *  V_F * Xtr(8,5)
                                        a_st_temp(:,k0,6).' * mm6 *  V_F * Xtr(1,6) , a_st_temp(:,k0,6).' * mm6 *  V_F * Xtr(2,6) , a_st_temp(:,k0,6).' * mm6 *  V_F * Xtr(3,6) , a_st_temp(:,k0,6).' * mm6 *  V_F * Xtr(4,6) , a_st_temp(:,k0,6).' * mm6 *  V_F * Xtr(5,6) , a_st_temp(:,k0,6).' * mm6 *  V_F * Xtr(6,6) , a_st_temp(:,k0,6).' * mm6 *  V_F * Xtr(7,6) , a_st_temp(:,k0,6).' * mm6 *  V_F * Xtr(8,6)
                                        a_st_temp(:,k0,7).' * mm7 *  V_F * Xtr(1,7) , a_st_temp(:,k0,7).' * mm7 *  V_F * Xtr(2,7) , a_st_temp(:,k0,7).' * mm7 *  V_F * Xtr(3,7) , a_st_temp(:,k0,7).' * mm7 *  V_F * Xtr(4,7) , a_st_temp(:,k0,7).' * mm7 *  V_F * Xtr(5,7) , a_st_temp(:,k0,7).' * mm7 *  V_F * Xtr(6,7) , a_st_temp(:,k0,7).' * mm7 *  V_F * Xtr(7,7) , a_st_temp(:,k0,7).' * mm7 *  V_F * Xtr(8,7)
                                        a_st_temp(:,k0,8).' * mm8 *  V_F * Xtr(1,8) , a_st_temp(:,k0,8).' * mm8 *  V_F * Xtr(2,8) , a_st_temp(:,k0,8).' * mm8 *  V_F * Xtr(3,8) , a_st_temp(:,k0,8).' * mm8 *  V_F * Xtr(4,8) , a_st_temp(:,k0,8).' * mm8 *  V_F * Xtr(5,8) , a_st_temp(:,k0,8).' * mm8 *  V_F * Xtr(6,8) , a_st_temp(:,k0,8).' * mm8 *  V_F * Xtr(7,8) , a_st_temp(:,k0,8).' * mm8 *  V_F * Xtr(8,8)                                        
                                        ];
...
tic
        ...
        m1 = find(s_temp(:,1) == 1); m2 = find(s_temp(:,2) == 1); m3 = find(s_temp(:,3) == 1); m4 = find(s_temp(:,4) == 1); m5 = find(s_temp(:,5) == 1); m6 = find(s_temp(:,6) == 1); m7 = find(s_temp(:,7) == 1); m8 = find(s_temp(:,8) == 1);

        if                 s_dic(m1,m2,1,2)  + s_dic(m1,m3,1,3)  + s_dic(m1,m4,1,4)  + s_dic(m1,m5,1,5)  + s_dic(m1,m6,1,6)  + s_dic(m1,m7,1,7) + s_dic(m1,m8,1,8) +...
                           s_dic(m2,m1,2,1)  + s_dic(m2,m3,2,3)  + s_dic(m2,m4,2,4)  + s_dic(m2,m5,2,5)  + s_dic(m2,m6,2,6)  + s_dic(m2,m7,2,7) + s_dic(m2,m8,2,8) +...
                           s_dic(m3,m1,3,1)  + s_dic(m3,m2,3,2)  + s_dic(m3,m4,3,4)  + s_dic(m3,m5,3,5)  + s_dic(m3,m6,3,6)  + s_dic(m3,m7,3,7) + s_dic(m3,m8,3,8) +...
                           s_dic(m4,m1,4,1)  + s_dic(m4,m2,4,2)  + s_dic(m4,m3,4,3)  + s_dic(m4,m5,4,5)  + s_dic(m4,m6,4,6)  + s_dic(m4,m7,4,7) + s_dic(m4,m8,4,8) +...
                           s_dic(m5,m1,5,1)  + s_dic(m5,m2,5,2)  + s_dic(m5,m3,5,3)  + s_dic(m5,m4,5,4)  + s_dic(m5,m6,5,6)  + s_dic(m5,m7,5,7) + s_dic(m5,m8,5,8) +...
