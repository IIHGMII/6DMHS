...
%% 针对初始6DMA位置，生成无线信道
[ h , e0 , e2 , theta0, phi0 , theta_scatterer0 , phi_scatterer0 , Iota , eta , Omega , h_r , h_c , h_sen , h_x_d , h_x_u , h_y_l , h_y_r ] = Channel_Generation_init( K,B,M,Mx,My,M_x_r , M_y_c , normal_vector,K_rice,Coor_Ele,Coor_Ele_r,Coor_Ele_c,lambda , Rot_Matrix , Coor_Ele_x_d,Coor_Ele_x_u,Coor_Ele_y_l,Coor_Ele_y_r);
%% 生成全息感知

% 所提感知算法
[est,est_L,est_error] = Sensing_Algorithm(K,B,Iota,Mx,My,M_x_r,M_y_c,h_r,h_c,Rot_Matrix,e0,K_rice,lambda,e2,sigma0,Omega,eta,N,d,kappa0,h_sen,h_x_d , h_x_u , h_y_l , h_y_r , Coor_Ele_init_x_d , Coor_Ele_init_x_u , Coor_Ele_init_y_l , Coor_Ele_init_y_r);
% 
% est_error


% est{1} = [  -0.058593750000000
%   -0.433593750000000
%    0.899201441514567];
% 
% est{2} = [   0.128906250000000
%    0.886718750000000
%   -0.443974140135859];
% 
% est{3} = [  -0.051363951070238
%    0.045030818644487
%    0.997664257103892];
% 
% est{4} = [  -0.644531250000000
%    0.714843750000000
%   -0.271252430144644];
...
for k = 1:K
    for iota = 0:Iota
        for b = 1:B
            ...
            if iota == 0
                    H( (b-1)*M+1 : b*M , k ) = H( (b-1)*M+1 : b*M , k ) + sig( normal_vector_rot{b}.' * e0{k} ) * sqrt( K_rice/(1+K_rice) ) * sqrt(Omega0) * exp( 1j * 2*pi/lambda * ( a1 * e0{k}  ) ) ;
            else
                    H( (b-1)*M+1 : b*M , k ) = H( (b-1)*M+1 : b*M , k ) + sig( normal_vector_rot{b}.' * e2{k,iota} ) * sqrt( 1/(1+K_rice) ) *  eta(k,iota) * exp( 1j * 2*pi/lambda * ( a1 * e2{k,iota} ) ) ;
            end
        end
    end
end
...
%-14
% 1.099647797873528e-04 4.541509602731497e-04

%-10
% 3.886971754451093e-05 0.001331902543782  3.166987606346738e-05
% 1.280506042659140e-04 0.000541161051878921	2.69567354263861e-05	8.02498353293468e-05


%2
% 0.001622805691311   0.001915390193503  0.001773697549062
% 4.603284712516724e-05 0.002609415512423 6.901898530172616e-04


%
% K_rice = [-10 -6 -2 2 6 10];
% Proposed

P_Proposed = [ 0.001331902543782      6.484117672122348e-04          0.001948836504182  0.002609415512423   0.003185009972614  0.003879553251756 ];


figure; plot(P_Proposed,'-o');
